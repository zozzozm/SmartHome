...
public class HSVColorPickerDialog extends AlertDialog {
    ...
    private static  int prevSelectedColor;
    ...
    public interface OnColorSelectedListener {
         ...
         */
        void OnDone(Integer color);
        void OnCancel(Integer color);
        void OnColorSelect(Integer color);
    }
    ...
    public HSVColorPickerDialog(Context context, int initialColor, final OnColorSelectedListener listener) {
        ...
        prevSelectedColor = initialColor;
        this.listener = listener;
       // Log.e("init Color", "HSVColorPickerDialog: R:"+Color.red(initialColor)+" G:"+Color.green(initialColor)+" B:"+Color.blue(initialColor)+ " A:"+Color.alpha(initialColor) );
        ...
        colorWheel.setListener( new OnColorSelectedListener() {
            ...
            public void OnDone(Integer color) {
                valueSlider.setColor( color, true );
            }

            @Override
            public void OnCancel(Integer color) {
        ...
        valueSlider.setListener( new OnColorSelectedListener() {
            ...
            public void OnDone(Integer color) {
                ...
                clr.setText("R:"+Color.red(selectedColor)+" G:"+Color.green(selectedColor)+" B:"+Color.blue(selectedColor));
            }
        ...
        clr = new TextView(getContext());
        clr.setText("R:"+Color.red(initialColor)+" G:"+Color.green(initialColor)+" B:"+Color.blue(initialColor));
        layout.addView(clr);
        ...
        public void onClick(DialogInterface dialog, int which) {
            switch ( which ) {
                case BUTTON_NEGATIVE:
                    listener.OnCancel( prevSelectedColor );
                    //dialog.dismiss();
                    break;
                case BUTTON_NEUTRAL:
                    dialog.dismiss();
     ...
     * which will call the listener's {@link OnColorSelectedListener#OnDone(Integer) colorSelected(Integer)} callback
     * with null as its parameter
     * @param res A string resource with the text to be used on this button
     */
    private void setNoColorButton( int res ) {
 ...
 int borderWidth = 50;
 Bitmap dstBitmap = Bitmap.createBitmap(w,h, Config.ARGB_8888);
 ...
 canvas.drawCircle(
         canvas.getWidth() >> 1, // cx
         canvas.getWidth() >> 1, // cy
         (canvas.getWidth() >> 1) - (borderWidth >> 1) + 42,//Radius
         paint // Paint
 );
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            switch ( action ) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if ( listener != null ) {
                        listener.OnDone( getColorForPoint( (int)event.getX(), (int)event.getY(), colorHsv ) );
                    }
                    invalidate();
                    return true;
    ...
    private static class HSVValueSlider extends View {
        ...
        public void setColor( int color, boolean keepValue ) {
            ...
            if ( listener != null ) {
                listener.OnDone( Color.HSVToColor( colorHsv ) );
            }
            ...
            switch ( action ) {
                ...
                case MotionEvent.ACTION_UP:
                    if ( listener != null ) {
                        ...
                        listener.OnColorSelect(Color.HSVToColor(colorHsv));
                    }
